...
export function SimulationPrintReport({ simulations }: SimulationPrintReportProps) {
  ...
  const groupedSims = useMemo(() => {
    const groups: Record<string, SimulationResult[]> = {}

    simulations.forEach((sim) => {
      if (!sim.farmIds || sim.farmIds.length === 0) {
        if (!groups['Simulações Gerais (Sem Vínculo)'])
          groups['Simulações Gerais (Sem Vínculo)'] = []
        groups['Simulações Gerais (Sem Vínculo)'].push(sim)
      } else {
        sim.farmIds.forEach((fId) => {
    ...
    <div className="print-report-container hidden print:block bg-white text-black p-8 font-sans w-full max-w-[210mm] mx-auto">
      <div className="border-b-2 border-primary pb-4 mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-gray-500 mt-1">Gestão Pecuária Integrada - Agrupado por Unidade</p>
        </div>
      ...
      <div className="mb-6">
        <p className="text-sm text-gray-600 mb-4">
          ...
          agrupadas por fazenda vinculada, incluindo os parâmetros de entrada, cotações de mercado e
          projeção de lucros.
        </p>
      </div>

      {Object.entries(groupedSims).map(([groupName, sims]) => (
        <div key={groupName} className="mb-10">
          <div className="bg-gray-100 border border-gray-200 px-4 py-2 mb-4 rounded-md">
            ...
            {sims.map((sim, idx) => (
              ...
              >
                <div className="flex justify-between items-center border-b border-gray-100 pb-3 mb-4">
                  <h3 className="text-lg font-bold text-gray-800">Cenário: {sim.category}</h3>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div>
                    ...
                    <ul className="space-y-2 text-sm">
                      ...
                      <li className="flex justify-between">
                        <span className="text-gray-600">Custo Base/Cab:</span>
                        <span className="font-semibold text-rose-600">
                          R${' '}
                          {sim.productionCost.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                        </span>
                      </li>
                      {sim.farmCost && sim.farmCost > 0 ? (
                        <li className="flex justify-between">
                          <span className="text-gray-600">Custo Fazenda/Cab:</span>
                          <span className="font-semibold text-rose-600">
                            R$ {sim.farmCost.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
                          </span>
                        </li>
                      ) : null}
                    </ul>
