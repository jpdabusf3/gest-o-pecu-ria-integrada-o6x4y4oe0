...
import { replacementIndicators } from '@/data/market'
import { TrendingDown, TrendingUp, Minus, ArrowRightLeft } from 'lucide-react'
import { cn } from '@/lib/utils'

export function ReplacementFilter() {
  const [selected, setSelected] = useState(replacementIndicators[0].id)

  const activeData =
    replacementIndicators.find((r) => r.id === selected) || replacementIndicators[0]
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-4 rounded-lg border bg-primary/5 border-primary/20 flex flex-col items-center justify-center">
          ...
          <div
            className={cn(
              'mt-2 text-xs font-medium flex items-center gap-1 px-2.5 py-1 rounded-full bg-background border',
              activeData.trend === 'up'
